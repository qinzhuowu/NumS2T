...
input_lang,output_lang,train_pairs,test_pairs,category_vocab,hownet_dict_vocab=prepare_data(pairs, pairs_from_test,
...
encoder = EncoderSeq(input_size=input_lang.n_words, embedding_size=embedding_size, hidden_size=hidden_size,
                     category_size=len(category_vocab),n_layers=n_layers)
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, 
    ...
    merge_optimizer,pivot_optimizer,SenLevel_optimizer, output_lang, num_pos,nums,category_index_batch,category_match_batch,
    ...
    encoder_outputs, problem_output,gate_num,num_indicator,just_problem_out,type_num,num_pair_score=encoder(input_var,
        input_length,cate_word_edge,cate_index_input,cate_length,cate_id_match,nums_index,num_pos)#B*cate+seq*cate+seq
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge,pivot, output_lang,
    num_pos,nums,category_index_list,category_match_list,output_middle_batch,input_edge_batch,hownet_dict_vocab,beam_size=3, english=False, max_length=Max_Expression_len):
    ...
    # 
    encoder_outputs, problem_output,gate_num,num_indicator ,just_problem_out,type_num,num_pair_score= encoder(input_var, 
        [input_length],cate_word_edge,cate_index_input,cate_length,cate_id_match,nums_index,[num_pos])
...
for epoch in range(start_epoch,n_epochs):
    ...
    for idx in range(start_epoch,len(input_lengths)):
        ...
        loss = train_tree(input_batches[idx], input_lengths[idx], output_batches[idx], output_lengths[idx],
            ...
            encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer,pivot_optimizer,SenLevel_optimizer, output_lang, 
